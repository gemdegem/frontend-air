...
export default function ModuleDetailsModal({
    ...
    return (
        ...
        >
            ...
            {/* <span className={classes.address} style={{ marginBottom: '0.5rem' }}><strong style={{ fontSize: '1.1rem', marginRight: '0.3rem' }}>Address:</strong>{moduleDetails.address}</span>
            <div className={classNames(classes.name)} style={{
                marginBottom: '0.3rem', marginTop: '1rem', borderLeft: '5px solid green',
                padding: '10px 0 10px 25px',
                background: '#e0ffd1'
            }}>Name: {moduleDetails.name}</div> */}
...
function InputElement({
    ...
    const formStyle = type === 'bool' ? { display: 'flex', alignItems: 'center' } : { display: 'flex', justifyContent: 'space-between', flexDirection: 'column' };

    return (
        <form className={classNames(classes.inputWrapper)} style={formStyle}>
